 ...
 */
import image from '@/img/icons/nicEditIcons-latest.gif';
...
window.onload = function () {
  ...
  if (params.indexOf("##") != -1) {
    var n1 = params.indexOf("/helpitems/") + 11;
    params = decodeURIComponent(params.substring(n1));
    //console.info(params);
    var elem = document.getElementById(params)
    console.info(elem);
    if (elem != null && !elem.isVisible) {
      //elem.scrollIntoView(false);  - не всегда срабатывает
      var targetOffset = elem.offsetTop;  //смещение метки, для скролла
      window.scrollTo(0, targetOffset - 40); // 40px - height of header
    }
  }
}
...
var nicEditorConfig = bkClass.extend({
  buttons: {
    ...
    'ol': {name: __('Добавление нумерованного списка'), command: 'insertorderedlist', tags: ['OL']},
    'ul': {name: __('Добавление перечисления'), command: 'insertunorderedlist', tags: ['UL']},
    ...
    'outdent': {name: __('Убрать отступ'), command: 'outdent', noActive: true},
    'hr': {name: __('Горизонтальная линия'), command: 'insertHorizontalRule', noActive: true}
  },
  buttonList: ['save', 'bold', 'italic', 'underline', 'subscript', 'superscript', 'strikethrough', 'left', 'center', 'right', 'justify', 'ol', 'ul', 'fontSize', 'fontFamily', 'fontFormat', 'indent', 'outdent', 'upload', 'link', 'unlink', 'forecolor', 'bgcolor', 'hr', 'removeformat', 'label'],
  iconList: {
    "xhtml": 1,
    "bgcolor": 2,
    "forecolor": 3,
...
var nicEditorInstance = bkClass.extend({
  ...
  init: function () {
    ...
    this.elm.addEvent('mousedown', this.selected.closureListener(this)).addEvent('keypress', this.keyDown.closureListener(this)).addEvent('keyup', this.keyUp.closureListener(this)).addEvent('focus', this.selected.closure(this)).addEvent('blur', this.blur.closure(this));
  ...
  keyDown: function (e, t) {
    if (e.ctrlKey) {
      this.ne.fireEvent('keyCtrl', this, e);
    }
  },
...
var nicEditorButton = bkClass.extend({
  ...
  construct: function (e, buttonName, options, nicEditor) {
    ...
    this.button = new bkElement('DIV').setStyle({
      ...
      zoom: 1,
      cursor: 'pointer'
    }).addClass('button nicEditIcons-latest ').setStyle(this.ne.getIcon(buttonName, options)).appendTo(this.border);
    ...
    nicEditor.addEvent('selected', this.enable.closure(this)).addEvent('blur', this.disable.closure(this)).addEvent('key', this.key.closure(this)).addEvent('keyCtrl', this.keyCtrl.closure(this));
  ...
  checkNodes: function (e) {
    ...
    do {
      ...
      elm = elm.parentNode;
    } while (elm && elm.className.indexOf("nicEdit") == -1);
  ...
  keyCtrl: function (nicInstance, e) {
  ...
  key: function (nicInstance, e) {
    if (!(this.options.key && e.ctrlKey && String.fromCharCode(e.keyCode || e.charCode).toLowerCase() == this.options.key)) {
      //индикация стилей соотв. кнопками на тулбаре
      var curNode = window.getSelection().getRangeAt(0).commonAncestorContainer;
      this.checkNodes(curNode.parentNode);
    }
  }
...
var nicSelectOptions = {
  buttons: {
    ...
    'fontFormat': {name: __('Выбрать формат текста'), type: 'nicEditorFontFormatSelect', command: 'formatBlock'}
  }
};
/* END CONFIG */
var nicEditorSelect = bkClass.extend({
  ...
  construct: function (e, buttonName, options, nicEditor) {
    ...
    this.ne.addEvent('selected', this.enable.closure(this)).addEvent('blur', this.disable.closure(this)).addEvent('key', this.key.closure(this));
    ;
  ...
  checkNodes: function (e) {
    ...
    do {
      ...
      switch (this.name) {
        case 'fontSize':
          if (elm.outerHTML && elm.outerHTML.indexOf("<font") != -1)
            strTag = elm.getAttribute('size');
          break;
        case 'fontFamily':
          if (elm.outerHTML && elm.outerHTML.indexOf("<font") != -1)
            strTag = elm.getAttribute('face');
          break;
        case 'fontFormat':
          if (this.sel[elm.localName] != null) //проверяем наличие в выпадающем списке, иначе выводит "undefine"
            strTag = elm.localName;
          break;
      }

      if (strTag != null) {
        //console.log(strTag);
        this.showStyle(strTag);
        flagHide = false;
      }
...
var nicLinkOptions = {
  buttons: {
    ...
    'unlink': {name: 'Убрать ссылку', type: 'nicUnLinkButton', noClose: true}
  }
};
/* END CONFIG */


var nicUnLinkButton = nicEditorButton.extend({
  mouseClick: function () {
    var ln = window.getSelection().getRangeAt(0);
    var selectionContents = ln.cloneContents();
    if (selectionContents.childNodes) {
      var selectedNode = selectionContents.childNodes[0];
      if (selectedNode.nodeName == "A") {
        var selectedInParent = this.findElm('A', 'href', selectedNode.getAttribute('href'));
        selectedInParent.parentNode.removeChild(selectedInParent);
      } else {  //проверяем, возможно выделился текст внутри элемента-ссылки
        var parentNode = ln.commonAncestorContainer;
        if (parentNode.nodeType != 1) {
          parentNode = parentNode.parentNode;
        }
        if (parentNode.nodeName == "A") {
          var selectedInParent = this.findElm('A', 'href', parentNode.getAttribute('href'));
          selectedInParent.parentNode.removeChild(selectedInParent);
        }
      }
    }
  ...
  findElm: function (tag, attr, val) {
    var list = nicEditors.findEditor('nicEditInstance').getElm().getElementsByTagName(tag);
    for (var i = 0; i < list.length; i++) {
      if (list[i].getAttribute(attr) == val) {
        return $BK(list[i]);
      }
...
var nicLinkButton = nicEditorButton.extend({
  mouseClick: function () {
    if (!this.isDisabled) {
      nicEditors.findEditor('nicEditInstance').saveRng();
      this.ln = null;//this.ne.selectedInstance.selElm().parentTag('A');
      Event.$emit('niceditOnLinkDialog');  //событие для вызова диалога выбора ссылки в nicedit.vue
     // zAu.send(new zk.Event(zk.Widget.$('$nicEditContainer'), 'onLinkDialog', null));
    }
  },
  ...
  submit: function (url, title) {
    if (url == "http://" || url == "") {
      alert("Необходимо выбрать ссылку");
      return false;
    }
    nicEditors.findEditor('nicEditInstance').restoreRng();

    if (!this.ln) {
      var tmp = 'javascript:nicTemp();';
      nicEditors.findEditor('nicEditInstance').nicCommand("insertHTML", '<A href="' + tmp + '">TMP</A>');
...
var nicColorOptions = {
  buttons: {
    'forecolor': {name: __('Изменить цвет'), type: 'nicEditorColorButton', noClose: true},
    'bgcolor': {name: __('Изменить фон'), type: 'nicEditorBgColorButton', noClose: true}
  }
};
/* END CONFIG */
...
var nicImageOptions = {
  buttons: {
    'image': {name: 'Добавить рисунок', type: 'nicImageButton', tags: ['IMG']}
  }
...
var nicSaveOptions = {
  buttons: {
    'save': {name: __('Сохранить содержимое'), type: 'nicEditorSaveButton'}
  }
};
/* END CONFIG */
...
var nicUploadOptions = {
  buttons: {
    'upload': {name: 'Загрузить рисунок', type: 'nicUploadButton'}
  }
...
var nicUploadButton = nicEditorAdvancedButton.extend({
  nicURI: null,
  errorText: 'Ошибка при загрузке рисунка',

  addPane: function () {
    if (typeof window.FormData === "undefined") {
      return this.onError("Загрузка рисунков не поддерживается браузером.");
    }
    this.im = this.ne.selectedInstance.selElm().parentTag('IMG');
  ...
  onError: function (msg) {
    this.removePane();
    alert(msg || "Ошибка при загрузке рисунка");
  },

  uploadFile: function () {
    var file = this.fileInput.files[0];
    if (!file || !file.type.match(/image.*/)) {
      this.onError("Могут быть загружены только файлы изображений");
      return;
    }
    ...
    fd.append("helpItemKey", this.ne.options.helpItemKey);

    API('helps/').request('upload/image', {
      data: fd,
      options: {
        headers: { 'Content-Type': 'multipart/form-data' }
      }
    }).then(x => {
        let data = x.data ? x.data : x;
        this.onUploaded(data);
    }).catch(e => {
      console.log(e);
      return this.onError();
    });
    /*  было, заменено вызовом API('helps/')
...
var nicLabelOptions = {
  buttons: {
    'label': {name: 'Добавить метку', type: 'nicLabelButton'}
  }
};
/* END CONFIG */

var nicLabelButton = nicEditorButton.extend({
  mouseClick: function () {
    if (!this.isDisabled) {
      nicEditors.findEditor('nicEditInstance').saveRng();
      Event.$emit('niceditOnLabelDialog');  //событие для вызова диалога выбора метки в nicedit.vue
      //zAu.send(new zk.Event(zk.Widget.$('$nicEditContainer'), 'onLabelDialog', null));
    }
  },
  submit: function (title, parentItemKey, pageKey) {
    var id = parentItemKey + "##" + pageKey
    //console.info(id)

    if (title == "") {
      alert("Необходимо задать имя");
      return false;
    }

    var emptyFlag = nicEditors.findEditor('nicEditInstance').getContent() == '';
    //console.log(nicEditors.findEditor('nicEditInstance').getContent());

    nicEditors.findEditor('nicEditInstance').restoreRng();
    nicEditors.findEditor('nicEditInstance').nicCommand("insertHTML", '<span id="' + id + '" title="' + title + '" style="color:red;">#</span>');

    var r = nicEditors.findEditor('nicEditInstance').savedRange;
    if (emptyFlag) {
      r.setStart(r.startContainer, r.startOffset + 1);
      r.setEnd(r.startContainer, r.startOffset); // r.startOffset уже поменян
    }
...
nicEditors.registerPlugin(nicPlugin, nicLabelOptions);

function handlerSubmitLinkDialog(data) {
//  nicEditors.findEditor('nicEditInstance').elm.focus();
  nicEditors.findEditor('nicEditInstance').ne.nicPanel.findButton('link').submit(data.url, data.linkTitle);

}

function handlerSubmitLabelDialog(data) {
//  nicEditors.findEditor('nicEditInstance').elm.focus();
  nicEditors.findEditor('nicEditInstance').ne.nicPanel.findButton('label').submit(data.labelName, data.parentItemKey, data.key);

}

function niceditInitNicEditor() {
  var myNicEditor = new nicEditor();
      myNicEditor.setPanel('nicEditPanel');
      myNicEditor.addInstance('nicEditInstance');
      //nicEditors.findEditor('nicEditInstance').setContent("hello <img src ='" + Vue.http.options.root + "/helps/get/image?helpImageKey=um9lh1qkshmmud7h2kak981696'>");
      //nicEditors.findEditor('nicEditInstance').setContent("hello<img src = '../helps/get/image?helpImageKey=um9lh1qkshmmud7h2kak981696'>");

      //свои обработчики событий писать сюда
      Event.$on('niceditOnSubmitLinkDialog', (data)=> handlerSubmitLinkDialog(data));
      Event.$on('niceditOnSubmitLabelDialog', (data)=> handlerSubmitLabelDialog(data));

}

function niceditGetHtmlContent() {
  return nicEditors.findEditor('nicEditInstance').getContent()
}

function niceditSetHtmlContent(data){
  nicEditors.findEditor('nicEditInstance').setContent(data.htmlContent);
  nicEditors.findEditor('nicEditInstance').ne.options.helpItemKey = data.helpItemKey;
}
