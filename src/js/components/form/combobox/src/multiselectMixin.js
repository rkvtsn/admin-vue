...
export default {
  ...
  computed: {
    ...
    filteredOptions() {
      ...
      let options = this.data.concat ();

      /* istanbul ignore else */
      if (this.internalSearch) {
        options = this.groupValues
          ? this.filterAndFlat (options, normalizedSearch, this.optionTitle)
          : filterOptions (
              options,
              normalizedSearch,
              this.optionTitle,
      ...
      return options.slice (0, this.dataLimit);
    },
    valueKeys() {
      if (this.trackBy) {
    ...
    optionKeys() {
      const options = this.groupValues
        ? this.flatAndStrip (this.data)
        : this.data;
      return options.map (element =>
        this.customLabel (element, this.optionTitle).toString ().toLowerCase ()
      );
    },
    currentOptionLabel() {
  ...
  watch: {
    ...
    'data.length'(len) {
      if (
        len > 0 &&
        (this.value === null || (isArray (this.value) && this.value.length))
      ) {
        this.preselect ();
      }
    },
  ...
  methods: {
    ...
    getValue() {
      return this.multiple
        ? this.internalValue
        : this.internalValue.length === 0 ? null : this.internalValue[0];
    },
    /**
    ...
    isExistingOption(query) {
      return !this.data ? false : this.optionKeys.indexOf (query) > -1;
    },
    /**
    ...
    getOptionLabel(option) {
      ...
      let label = this.customLabel (option, this.optionTitle);
      /* istanbul ignore else */
    ...
    selectGroup(selectedGroup) {
      const group = this.data.find (option => {
