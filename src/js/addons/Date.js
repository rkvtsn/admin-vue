...
export default (function() {
  ...
  (function() {
    ...
    /**
     * Default template for formating the date.
     * @static
     * @property {String} defaultFormat
     */
    Date.defaultFormat = 'd.m.Y H:i:s';


// create private copy of String.format() method
...
// - to resolve namespace conflict with M$-Ajax's implementation
    ...
    function formatCodeToRegex(character, currentGroup) {
      ...
      return p
          ? applyIf({c: p.c ? xf(p.c, currentGroup || "{0}") : p.c}, p)
          : {
            g: 0,
            c: null,
...
// shorthand for formatCodeToRegex since we'll be using it fairly often
    var $f = formatCodeToRegex;
    ...
    apply(Date, {
       ...
       * @property {Object} parseFunctions
       * @static
       */
      parseFunctions: {
      ...
      formatCodes: {
        ...
        /**
         * calc. See {@link #parseCodes}, {@link #createParser}.
         * @return {string}
         */
      ...
      isValid: function(year, month, day, hour, minute, second, millisecond) {
        ...
        var dt = new Date(year < 100 ? 100 : year, month - 1, day, hour, minute, second, millisecond)
            .add(Date.YEAR, year < 100 ? year - 100 : 0);
      ...
      parseDate: function(input, format, strict) {
        format = format || Date.defaultFormat;
      ...
      createParser: function() {
        var code = [
          ...
          // date calculations (note: these calculations create a dependency on parseNumber())
          "y = parseNumber(y, parseNumber(def.y, dt.getFullYear()));",
          "m = parseNumber(m, parseNumber(def.m - 1, dt.getMonth()));",
          "d = parseNumber(d, parseNumber(def.d, dt.getDate()));",

          // time calculations (note: these calculations create a dependency on parseNumber())
          "h  = parseNumber(h, parseNumber(def.h, dt.getHours()));",
          "i  = parseNumber(i, parseNumber(def.i, dt.getMinutes()));",
          "s  = parseNumber(s, parseNumber(def.s, dt.getSeconds()));",
          "ms = parseNumber(ms, parseNumber(def.ms, dt.getMilliseconds()));",
  ...
  apply(Date.prototype, {
     ...
     * @returns {Date}
     */
    dateFormat: function(format = Date.defaultFormat) {
    ...
    getTimezone: function() {
      ...
      return this.toString().replace(/^.* (?:\((.*)\)|([A-Z]{1,4})(?:[-+][0-9]{4})?(?: -?\d+)?)$/, "$1$2").replace(
          /[^A-Z]/g,
          "");
    },
  ...
  /**
   * Formats a date given the supplied format string. See {@link Date#dateFormat}.
